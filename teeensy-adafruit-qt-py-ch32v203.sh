...
#
# teeensy-adafruit-qt-py-ch32v203.sh (super simple sample code)
# this code for Adafruit QT Py CH32V203 will turn on the NEOPIXEL at PA4
# more information is available at https://www.adafruit.com/products/5996
#
...
_start:
  ...
  li t0, 0x44434444 /* MODE4[1:0], CNF4[1:0] setup, others reset default */
  li t1, 0x40010800 /* R32_GPIOA_CFGLR */
  sw t0, 0(t1)
  ...
  li a2, 1 << 4
  li a3, 1 << 20
  li a4, 0x40010810 /* R32_GPIOA_BSHR */

  /* when programming the device over USB and booting into the new */
  /* software it seems that the state of the pin PA2 is changing, */
  /* this might not apply to pin PA4 and also it might not happen */
  /* when booting without USB. regardless it makes sense to always */
  /* start from a known state. because of that set the pin to 1 here */
...
#
# after connecting USB-C the device needs to be rebooted into the boot loader
# press RESET followed by BOOT and release RESET followed by BOOT
# the BOOT button is the one next to the STEMMA QT port
#
